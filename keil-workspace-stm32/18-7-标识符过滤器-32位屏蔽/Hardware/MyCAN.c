...
{	
  ...
  //         = 36MHz / (BRP[9:0]+1) / (1 + (TS1[3:0]+1) + (TS2[2:0]+1))
	...
	CAN_FilterInitStructure.CAN_FilterIdHigh = ID1 >> 16;	// 接收所有信息，不进行过滤
	CAN_FilterInitStructure.CAN_FilterIdLow = ID1;	// 低位放ID1是因为，正好IDE所在低位都位0
	
	uint32_t ID2 = (0x1FFFFF00<<3)|0x4|0x2;
