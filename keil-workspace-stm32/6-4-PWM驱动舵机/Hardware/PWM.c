...
设：PSC = 72 - 1
	...
	舵机范围， PWM占空比
	0.5ms -90度  	0.5/20    	CCR = 500
	  1ms -45度   	1/20 -45
	1.5ms - 0度  	1.5/20  0
	  2ms  45度		 	2/20  45
	2.5ms  90度	 	2.5/20  90		CCR = 2500
...
void PWM_init(void)
{
	...
	TIM_TimeBaseInitStructure.TIM_Period = 20000 - 1;								// 计数周期，即ARR的值 ARR：（Auto-Reload Register，自动重装载寄存器） ARR 是定时器的最大计数值。
